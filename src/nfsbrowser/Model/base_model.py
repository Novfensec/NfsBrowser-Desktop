...
class BaseScreenModel(EventDispatcher):
    ...
    def on_result(self, *args) -> None:
        if self.tcp.result[0] == None and self.tcp.result[1] != None:
            self.error = self.tcp.result[1]

    def on_error(self, instance: object, value: str | None, *args) -> None:
        if value != None:
            self.app.ban_state(True)
